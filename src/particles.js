...
class ParticlePlane {
  constructor(amount_x, amount_y, separation) {
    ...
    for (let ix = 0; ix < this.amount_x; ix++) {
      for (let iy = 0; iy < this.amount_y; iy++) {
        this.positions[i] =
          ix * this.separation - (this.amount_x * this.separation) / 2; // x
        this.positions[i + 1] = 0; // y
        this.positions[i + 2] =
          iy * this.separation - (this.amount_y * this.separation) / 2; // z
  ...
  update(count, rotateY = 0.0005) {
    ...
    for (let ix = 0; ix < this.amount_x; ix++) {
      for (let iy = 0; iy < this.amount_y; iy++) {
        positions[i + 1] =
          Math.sin((ix + count) * 0.3) * 50 + Math.cos((iy + count) * 0.5) * 50;
